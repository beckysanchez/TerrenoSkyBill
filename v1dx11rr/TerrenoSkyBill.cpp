...
LRESULT CALLBACK WindowProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(message)
    {
        ...
        case WM_MOUSEMOVE: {
            ...
            dxrr->arriaba = 0;
            dxrr->vel = 0;
			dxrr->lateral = 0; //agregamos esto para movernos lateralmente
            ...
            //arriba abajo son de lateral deaaa
            ...
            if (gamePad->IsConnected())
            {
                ...
                if (grados > 0.19 || grados < -0.19)dxrr->arriaba = grados / 95;
				...
				//aqui agregamos lo mismo para lateral
                float lateral = (float)gamePad->GetState().Gamepad.sThumbLX / 32767.0;
                if (lateral > 0.19 || lateral < -0.19) {
                    dxrr->lateral = -lateral * 1.0f;  
                }
                else {
                    dxrr->lateral = 0.0f;
                }
