...
public:	
	...
	float arriaba;
	float vel;
	float lateral;  //intentaremos agregar esto para ver si podemos movernos de izquierda a derecha
	bool breakpoint;
	vector2 uv1[32];
	vector2 uv2[32];
	vector2 uv3[32];
    ...
    DXRR(HWND hWnd, int Ancho, int Alto)
	{
		...
		backBufferTarget = 0;
		IniciaD3D(hWnd);
		//agregue este para que no aparezca el cursor que me da tock de solo verlo ahi en el medio de la nada deoooss
		ShowCursor(FALSE);
		izqder = 0;
		arriaba = 0;
		lateral = 0;// agregamos esto tmb para lo de izquierda a derecha pa
		billCargaFuego();
	...
	~DXRR()
	{
		//agregyue el true porque en dado caso de que ocupemos otro render pues que sea aja pues capaz lo ocupamos ver 
		ShowCursor(TRUE);
		LiberaD3D();
		m_XACT3.Terminate();
	}
	...
	void Render(void)
	{
		...
		camara->UpdateCam(vel, arriaba, izqder, lateral); //agregamos lateral 
